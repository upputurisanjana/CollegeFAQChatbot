...
def phase1_entity_store():
    ...
    ok = name3 == "bob"
    print(f"  [{'PASS' if ok else 'FAIL'}] lowercase 'bob' captured (regex IGNORECASE) -> got '{name3}'")
    if ok:
        passed += 1
...
def phase2_conversation_memory():
    print_header("PHASE 2: ConversationMemory - context injection & history truncation")
    ...
    has_history = any(m["role"] in ("user", "assistant") for m in prepared)
    ok = has_context and has_history
    print(f"  [{'PASS' if ok else 'FAIL'}] context injected + history preserved "
          f"(context={'YES' if has_context else 'NO'}, msgs={len(prepared)})")
    if ok:
        passed += 1
    ...
    has_recent = any("query 7" in m.get("content", "") for m in prepared_long)
...
def phase3_end_to_end():
    ...
    if remembers:
        passed += 1
    print(f"    {'PASS' if remembers else 'FAIL'} - model {'remembered' if remembers else 'did not use'} the name")
